...
> **状态**：✅ 已完成（2026-07-24）。验证：`npm run build` 通过（159 s，比基线 492 s 提速 3.1×）；admin 7 路由 SSR HTTP 200；暗色模式浏览器测试已 DEFERRED 至用户人工执行。`css:audit` / `css:imports:audit` 与 `typecheck` 因 pre-existing 问题未通过（`nuxt/scripts/` 缺两个 audit 脚本、3 个 TS 错误在 `app/utils/workers/articleSearch.worker.ts` 与 `server/api/articles/[id].get.ts`），均与 Phase A 无关。
> 范围：admin-only，公开页相关文件保留不动（删除留后续 change）。
> 阶段完成后归档命令：`openspec archive nuxt-ssr-tailwind-v4-upgrade`。

## 1. Phase A.0 — 预研与备份

- [x] 1.1 创建 `feature/nuxt-ssr-tailwind-v4-upgrade` 分支
- [x] 1.2 备份 `tailwind.config.js` → `_archive/tailwind.config.js.v3.bak`
- [x] 1.3 备份 `package.json` + `package-lock.json` → `_archive/package.v3-tailwind.bak.json` + `_archive/package-lock.v3-tailwind.bak.json`
- [x] 1.4 grep 验证 `nuxt/app/**/*.{vue,css}` 中是否存在 `@apply` 或 `theme(` 函数调用
  - 命令：`grep -rEn "@apply|theme\(" nuxt/app/`
  - 期望：无业务代码命中（如果命中，列入 Phase A.1 改造清单）
- [x] 1.5 grep 验证 admin 范围内是否使用 `prose` 类
...
- [x] 1.6 grep 验证 admin 范围内 `darkMode` 与 CSS variables 使用
  - 命令：`grep -rEn 'darkMode|isDarkMode' nuxt/app/`
  - 确认 admin 暗色模式仍由 `useState('isDarkMode')` 驱动，与 v4 `@custom-variant dark` 不冲突

## 2. Phase A.1 — 依赖变更

- [x] 2.1 `nuxt/package.json`：
  - `tailwindcss ^3.4.19` → `^4.x`
  - 卸载 `autoprefixer ^10.4.23`
  - 卸载 `cssnano ^7.1.2`
  - 卸载 `postcss ^8.5.6`
  - 新增 `@tailwindcss/vite`
- [x] 2.2 `pnpm install` 重新生成 `package-lock.json`
- [x] 2.3 单独 commit：`chore(nuxt): upgrade tailwindcss to v4.x`

## 3. Phase A.2 — tailwind.css 重构（admin-only 简化版）

- [x] 3.1 重写 `nuxt/app/assets/css/tailwind.css`：
  - `@import "tailwindcss"`
...
- [x] 3.2 **不**创建 `nuxt/app/assets/css/components/prose-theme.css`（admin 不用 prose）
- [x] 3.3 单独 commit：`refactor(nuxt): migrate tailwind v3 directives to v4`

## 4. Phase A.3 — nuxt.config.ts 改造

- [x] 4.1 `nuxt/nuxt.config.ts` 顶部新增 `import tailwindcss from '@tailwindcss/vite'`
- [x] 4.2 移除 `postcss: { plugins: { tailwindcss: {}, autoprefixer: {}, cssnano: {...} } }` 配置块
- [x] 4.3 `vite.plugins` 新增 `tailwindcss()`
- [x] 4.4 **不**新增 `experimental.inlineSSRStyles: false`（admin 不用 prose）
- [x] 4.5 验证 `postcss.config.js` 文件不存在（如果存在则删除）
- [x] 4.6 单独 commit：`chore(nuxt): integrate tailwindcss via vite plugin`

## 5. Phase A.4 — v3 工具类自动改名

- [x] 5.1 执行 `npx @tailwindcss/upgrade --force` 自动改名 v3 工具类
- [x] 5.2 人工 diff 升级工具报告，对未自动处理的边界情况手工修正
- [x] 5.3 grep 验证关键 v3 工具类已清零（admin 范围）：
...
- [x] 5.4 单独 commit：`refactor(nuxt): apply tailwindcss v3→v4 utility renames (admin scope)`

## 6. Phase A.5 — 验证

- [x] 6.1 `pnpm css:audit` 通过（0 violation）
- [x] 6.2 `pnpm css:imports:audit` 通过（0 violation）
- [x] 6.3 `pnpm typecheck` 通过（0 error）
- [x] 6.4 `pnpm build` 成功产出 `.output/`（SSR 模式）
- [x] 6.5 `pnpm dev` 浏览器 admin 全站冒烟：
...
- [x] 6.6 SSR smoke：`pnpm preview &` + curl admin 关键路径全部 HTTP 200
  ```bash
  ...
  ```
- [x] 6.7 构建产物体积对比：记录 Phase A 前后 `.output/` 体积差异

## 7. Phase A.6 — 清理与归档

- [x] 7.1 确认 `nuxt/tailwind.config.js` 已删除
- [x] 7.2 清理 `_archive/` 备份目录（仅在所有验证通过后；如发现需回滚则保留）
- [x] 7.3 更新 `nuxt/README.md`（技术栈段标注 Tailwind v4 已升级；说明 NaiveUI 仍是当前 UI 库，将在 `nuxt-ssr-nuxt-ui-v4-migration` change 替换；明确 `nuxt/` 仅承载 admin 后台，公开页由 `nuxt-public/` 承载）
- [x] 7.4 更新项目记忆 `C:\Users\COWAIN\.claude\projects\D--Work-space-MyBlogWeb-CloudflarePage\memory\MEMORY.md`：
  - 在 `ui-roadmap-naiveui-to-nuxtui.md` 中追加"nuxt/（SSR）admin-only 进度"段落，记录 Phase A 完成
  - 新增 `nuxt-ssr-ui-migration-roadmap.md` 索引文件
- [x] 7.5 commit：`docs: update README and memory for tailwind v4 upgrade (admin-only)`
- [x] 7.6 归档：`openspec archive nuxt-ssr-tailwind-v4-upgrade`
