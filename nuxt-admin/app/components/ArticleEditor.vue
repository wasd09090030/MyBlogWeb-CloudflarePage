...
<template>
  <div :class="['space-y-5', { 'fixed inset-0 z-50 overflow-auto bg-default p-5': fullscreen }]">
    ...
    <div class="flex flex-wrap items-center gap-2 border-y border-default py-2">
      ...
      <span class="text-xs font-medium text-muted">MDC</span>
    ...
    <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <UCard class="h-fit xl:sticky xl:top-20"><div class="space-y-4"><UFormField label="标题" required><UInput v-model="form.title" class="w-full" /></UFormField><UFormField label="Slug"><UInput v-model="form.slug" class="w-full" /></UFormField><UFormField label="分类"><USelect v-model="form.category" :items="['study', 'game', 'work', 'resource', 'other']" class="w-full" /></UFormField><UFormField label="封面图"><UInput v-model="form.coverImage" class="w-full" /></UFormField><UFormField label="标签"><UInputTags v-model="form.tags" class="w-full" /></UFormField><UFormField label="AI 摘要"><UTextarea v-model="form.aiSummary" :rows="4" class="w-full" /></UFormField><UButton block variant="soft" :loading="aiPending" icon="i-lucide-sparkles" @click="aiSummary">生成 AI 摘要</UButton><USeparator /><p class="text-sm text-muted">{{ stats.characters }} 字符 · {{ stats.words }} 词 · {{ stats.headings }} 标题</p><p v-if="draftSavedAt" class="text-xs text-muted">草稿已保存</p></div></UCard>
    </div>
